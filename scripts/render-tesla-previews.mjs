...
const origin = process.env.TESLA_PREVIEW_ORIGIN || 'http://localhost:5180'
const manifest = JSON.parse(await readFile(new URL('../src/tesla/manifest.json', import.meta.url), 'utf8'))
await mkdir(new URL('../public/models/tesla/', import.meta.url), { recursive: true })
...
try {
  ...
  for (const id of Object.keys(manifest)) {
    ...
    await sharp(bytes).resize(900, 600).webp({ quality: 94, alphaQuality: 100 }).toFile(new URL(`../public/models/tesla/${id}.webp`, import.meta.url).pathname)
