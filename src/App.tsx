...
function getRoute(): Route {
  ...
  if (tab === 'games') return { tab, detail: detail === 'starflight' ? detail : undefined }
...
export default function App() {
  ...
  useEffect(() => { const detailName = route.tab === 'models' && route.detail ? assetById[route.detail as AssetId].name : route.tab === 'scenes' && route.detail ? '静海发射基地' : route.tab === 'games' && route.detail ? '星际穿行' : ''; document.title = `${detailName ? `${detailName} · ` : ''}${tabLabels[route.tab]} · 形际 FORM / SPACE` }, [route])
  ...
  return <div className={`app-shell ${route.tab === 'games' && route.detail ? 'combat-route' : ''}`}>
    ...
    <header className="site-header"><div className="header-inner"><button className="brand" aria-label="形际首页" onClick={() => navigate('models')}><OrbitLogo /><span><strong>形际</strong><small>FORM / SPACE</small></span></button><nav aria-label="主导航">{(['models', 'scenes', 'games'] as Tab[]).map(tab => <button key={tab} aria-current={route.tab === tab ? 'page' : undefined} className={route.tab === tab ? 'active' : ''} onClick={() => navigate(tab)}>{tabLabels[tab]}</button>)}</nav><span className="header-tagline">你的三维游乐场<span className="header-orbit" /></span></div></header>
    <main id="main-content" tabIndex={-1} className={`main-content ${route.detail ? 'detail-main' : ''}`}>
      {route.tab === 'models' && !route.detail && <ModelLibrary selected={selected} toggleSelection={toggleSelection} viewModel={viewModel} playGame={play} addSelected={() => addAssets()} />}
      {route.tab === 'models' && route.detail && <ModelWorkspace key={route.detail} id={route.detail as AssetId} selected={selected.includes(route.detail as AssetId)} toggleSelection={() => toggleSelection(route.detail as AssetId)} back={() => navigate('models')} play={play} add={() => addAssets([route.detail as AssetId])} />}
      ...
      {route.tab === 'games' && !route.detail && <GameLibrary playGame={play} viewModel={viewModel} />}
      {route.tab === 'games' && route.detail && <FlightGame back={() => navigate('games')} viewModel={() => viewModel('rocket')} />}
    </main>
    <footer className="site-footer"><span>FORM <i>/</i> SPACE</span><span>让想象，成为可以触碰的世界。</span></footer>
