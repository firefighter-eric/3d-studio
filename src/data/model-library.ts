...
import { nvidiaProduct, nvidiaPreviewUrl, nvidiaModelUrl, isNvidiaProductId } from '../nvidia/specs'
import { isSpacecraftId } from '../spacecraft/specs'
import { isCarId } from '../racing/cars'

export const MODEL_BRANDS = { spacex: 'SpaceX', apple: 'Apple', dji: 'DJI', nvidia: 'NVIDIA', original: '原创' } as const
export const MODEL_TYPES = {
  ...
  rack: '机柜', server: '服务器', gpu: 'GPU', car: '赛车', facility: '设施', nature: '自然',
} as const
...
const brandAliases: Record<ModelBrand, string> = {
  spacex: 'SpaceX 太空探索技术公司', apple: 'Apple 苹果', dji: 'DJI 大疆', nvidia: 'NVIDIA 英伟达 辉达 NV', original: '原创 3D Studio',
}
const typeAliases: Record<ModelType, string> = {
  ...
  headset: 'headset VR AR 空间计算', rack: 'rack NV72 NVL72 液冷', server: 'server 算力', gpu: 'GPU 显卡 芯片 SXM',
  car: 'car racing F1 方程式', facility: 'facility 建筑 基地', nature: 'nature rock 岩石',
}
...
function describeModel(asset: ModelAsset): LibraryModel {
  ...
  if (isNvidiaProductId(asset.id)) {
    ...
    return { ...asset, brand: 'nvidia', type: types[product.family], source: 'reconstructed',
      aliases: [...product.features, ...(asset.id === 'nvidia-dgx-station' ? ['NV Studio', 'NVIDIA Studio'] : [])],
  ...
  if (isCarId(asset.id)) return { ...asset, brand: 'original', type: 'car', source: 'original', aliases: ['方程式赛车'], download: `/models/${asset.id}.glb` }
...
const featured: AssetId[] = ['starship', 'apple-iphone-17-pro', 'dji-osmo-pocket-4p', 'nvidia-gb300-nvl72', 'apple-macbook-pro', 'nvidia-dgx-spark', 'dji-mavic-4-pro', 'falcon-9']
...
const searchIndex = new Map(libraryModels.map(model => [model.id, normalize([
  model.name, model.id, model.english, model.description, brandAliases[model.brand], MODEL_TYPES[model.type], typeAliases[model.type], ...model.aliases,
].join(' '))]))
...
export function filterModels(state: ModelLibraryState): LibraryModel[] {
  ...
  const result = libraryModels.filter(model => (state.brand === 'all' || model.brand === state.brand)
    ...
    && terms.every(term => searchIndex.get(model.id)!.includes(term)))
